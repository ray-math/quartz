...
수열이 $a_{n+1} = a_n f(n)$이라는 규칙을 따르는 경우를 생각해 봅시다. 이 규칙은 다음 항이 이전 항에 일정한 수가 아닌, 항의 번호 $n$에 따라 변하는 값 $f(n)$을 곱해서 만들어진다는 의미입니다. 이 수열의 일반항을 구하기 위해, 등비수열에서 했던 것과 비슷한 방법을 사용해 보겠습니다.
