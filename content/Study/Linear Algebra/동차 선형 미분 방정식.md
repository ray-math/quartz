...
- $n_1, n_2, \dots, n_k$는 양의 정수(positive integers)이며,
- $c_1, c_2, \dots, c_k$는 서로 다른(distinct)복소수이다.

그러면, 미분 방정식의 해 공간(solution space)의 기저(basis)는 다음과 같다.

$$
\{ e^{c_1 t}, t e^{c_1 t}, \cdots, t^{n_1 - 1} e^{c_1 t}, \cdots,
e^{c_k t}, t e^{c_k t}, \cdots, t^{n_k - 1} e^{c_k t} \}
$$




반복근(repeated roots)의 경우 해 공간의 기저는 단순한 $e^{ct}$ 형태가 아닌, 다항식 계수(polynomial coefficients)를 포함하는 형태가 된다.
